...
pub fn compile(source: &str, options: &CompileOptions) -> Result<(), String> {
    ...
    // Write IR to a temporary file, then invoke clang -c.
    let tmp_dir = std::env::temp_dir();
    let ir_path = tmp_dir.join("nimble_program.ll");
    let mut ir_file =
        std::fs::File::create(&ir_path).map_err(|e| format!("failed to create temp IR: {}", e))?;
    ir_file

...
use nimble::{ParseError, Parser, TypeError, TypeChecker};

/// Convert a `nimble::Span` into an LSP `Range`.
fn span_to_range(span: &nimble::Span, line_index: &[usize]) -> Range {
...
impl LanguageServer for Backend {
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        let line_index = build_line_index(&source);
        let byte_offset = line_index.get(pos.line as usize).copied().unwrap_or(0) + pos.character as usize;

        // Find the identifier token at this byte offset.
        let token_at_cursor = {
            let lexer = nimble::Lexer::new(&source);
        ...
        if let Some(tok) = token_at_cursor {
            if let nimble::TokenKind::Identifier(name) = &tok.kind {
